...
try {
    ...
    $requiredColumns = ['name', 'email', 'subject', 'message', 'created_at'];
    $optionalColumns = ['phone', 'company', 'service', 'budget'];
    ...
    echo "<h3>Optional Columns:</h3>";
    foreach ($optionalColumns as $col) {
        if (in_array($col, $columnNames)) {
            echo "<p style='color: green;'>✅ $col</p>";
        } else {
            echo "<p style='color: orange;'>⚠️ $col (not available)</p>";
        }
    }
    ...
    if (in_array('phone', $columnNames) && in_array('company', $columnNames) && in_array('service', $columnNames) && in_array('budget', $columnNames)) {
        $sql = "INSERT INTO contact_messages (name, email, phone, company, service, budget, subject, message, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        $params = [$testData['name'], $testData['email'], $testData['phone'], $testData['company'], $testData['service'], $testData['budget'], $testData['subject'], $testData['message']];
    } else {
        $sql = "INSERT INTO contact_messages (name, email, subject, message, created_at) VALUES (?, ?, ?, ?, NOW())";
        $params = [$testData['name'], $testData['email'], $testData['subject'], $testData['message']];
    }
...
echo "<ol>";
echo "<li>If any required columns are missing, run the SQL script: <code>fix-contact-messages-table.sql</code></li>";
echo "<li>Test the contact form: <a href='test-contact-final.html'>Test Contact Form</a></li>";
echo "<li>Test the main contact page: <a href='contact.php'>Contact Page</a></li>";
echo "</ol>";
?>
