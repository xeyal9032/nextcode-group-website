...
class PerformanceOptimizer {
    ...
    initWebWorkers() {
        // Web Workers disabled to comply with Content Security Policy
        // Blob URLs are blocked by CSP for security
        // All performance calculations run on main thread with optimizations
        this.worker = null;
    }
    ...
    splitLongTask(entry) {
        // Use requestIdleCallback to defer work when browser is idle
        if (this.config.enableIdleCallback && 'requestIdleCallback' in window) {
            requestIdleCallback(() => {
                // Perform cleanup or deferred operations
                this.performDeferredCleanup();
            }, { timeout: 5000 });
        } else {
