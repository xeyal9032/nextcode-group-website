...
// Production logging: Secure form data logging without sensitive data exposure
$logData = [
...
// Secure database connectivity check
if (!isset($pdo)) {
    error_log('Database connection not available');
    echo json_encode(['success' => false, 'message' => 'Server connectivity issue. Please try again.']);
    exit;
}

// Production: Validate database connectivity
try {
    $pdo->query('SELECT 1');
} catch (PDOException $e) {
    error_log('Database connection issue: ' . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Temporarily unavailable. Please try again later.']);
    exit;
}
