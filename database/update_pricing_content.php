<?php
/**
 * Pricing Content Database Update Script
 * Azerbaycan dilinde pricing sayfası içeriklerini veritabanına ekler
 */
...
try {
    ...
    echo "Pricing içerikleri veritabanına ekleniyor...\n";
    ...
    $stmt = $pdo->prepare("
        ...
        (content_key, content_value, content_type, page_section, description, is_active, sort_order) 
        VALUES (?, ?, ?, ?, ?, 1, 0)
    ");
    ...
    foreach ($pricingContent as $content) {
        try {
            ...
            echo "✗ Error inserting {$content[0]}: " . $e->getMessage() . "\n";
        }
    }
    
    echo "\n✅ Pricing içerikleri başarıyla eklendi!\n";
    echo "Toplam {$successCount} içerik güncellendi.\n";
    
    // Verify content
    echo "\nVeritabanındaki pricing içerikleri:\n";
    ...
    foreach ($results as $row) {
        echo "- {$row['content_key']}: " . substr($row['content_value'], 0, 50) . "...\n";
    }
